...
if args.pickup:
    ...
    if is_last_slice:
        ...
        thours = int(t/3600)
        chkfile = DumbCheckpoint(name+"_"+str(thours)+"h", mode=FILE_CREATE, comm=ensemble.comm)
        chkfile.store(un)
        chkfile.store(etan)
        chkfile.store(b)
        chkfile.store(PV)
...
chkfile = DumbCheckpoint(name+"_"+str(thours)+"h", mode=FILE_READ, comm=ensemble.comm)
chkfile.load(un, name="Velocity")
chkfile.load(etan, name="Elevation")
t = chkfile.read_attribute("/", "time")
tdump = chkfile.read_attribute("/", "tdump")
