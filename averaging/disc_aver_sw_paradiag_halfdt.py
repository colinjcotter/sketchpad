...
Wallh = asQ.AllAtOnceFunction(ensemble, time_partition_t, W)
...
forwardp_expProb_dt_half = LinearVariationalProblem(lhs(F1p), rhs(F1p), W1,
                                            constant_jacobian=constant_jacobian)
forwardp_expsolver_dt_half = LinearVariationalSolver(forwardp_expProb_dt_half,
                                                solver_parameters=params)
...
if paradiag_dt:
    ...
    propagate_form_half = asQ.AllAtOnceForm(Wallh, 0.5*dt/nt, theta,
                                            form_mass, get_form_function())
    ...
    if paradiag_dt:
        ...
        for step in ProgressBar(f'propagate').iter(range(nt)):
            with PETSc.Log.Event("forward propagation dt"):
                if half:
                    forwardp_expsolver_dt_half.solve()
                else:
